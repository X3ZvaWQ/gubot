        ...
        if (result == null) {
            ...
            if (args.rank < 1) {
                if (args.rank == -1) {
                    ...
                    macroId = recommandList.filter(
                        macro => macro.icon == kungfu.icon
                    )[0];
                    //如果找不到推荐宏的话就直接从排行榜取
            ...
            if (result == null) {
                ...
                let macros = post.post_meta.data.map(async macro => {
                    ...
                    if (macro.talent != "" && macro.talent != null) {
                        try {
                            ...
                            let talentList = await bot.redis.get(
                                redis_talent_key
                            );
                            if (talentList == null) {
                                talentList = await Jx3box.talentsList(
                                    talent.version || "v20201030"
                                );
                                talentList = talentList[talent.xf];
                                await bot.redis.set(
                                    redis_talent_key,
                                    JSON.stringify(talentList)
                                );
                            } else {
                ...
                result = `${CqHttp.imageCQCode(
                    await bot.imageGenerator.generateFromTemplateFile(
                        "macro",
                        data
                    )
                )}
