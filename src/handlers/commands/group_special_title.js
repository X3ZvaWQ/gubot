...
module.exports = class GroupSpecialTitleHandler {
    ...
    async handle(event) {
        ...
        const allow =
            (await group.getOption("allow_self_special_title")) == "true";
        if (!allow) return;
        let special_title = args.special_title;
        Logger.info(
            `${this.name}: allow group [${group.group_id}] special title request by qq user [${user.qq}]`
        );
        return [
            ...
            Cqhttp.sendGroupMessage(
                `${
                    user.nickname
                        ...
                        : data.user_id
                } 给自己带上了 ${special_title} 的帽子, 虽然不知道有没有成功`,
                group.group_id
            ),
        ];
    }
